...
class VendorCollector:
    def __init__(self, cfg: dict):
        ...
        self.passthrough_cfg = v.get("passthrough", {}) or {}
    ...
    # ---------- Live passthrough (Modbus cez cloud, 1 s) ----------
    _PT_GRID_V = 2
    _PT_OUT_FREQ = 3
    _PT_AC_POWER = 4
    _PT_BATT_V = 8
    ...
    def fetch_passthrough(self) -> dict | None:
        """Živé dáta z meniča cez remote/device/passthrough (Modbus string)."""
        if not self.enabled:
            return None
        pt = self.passthrough_cfg
        if not pt.get("enabled", False):
            return None
        if not self._ensure_token():
            return None
        device_id = str(pt.get("device_id") or "")
        b64_in = str(pt.get("base64_input") or "")
        if not device_id or not b64_in:
            return None
        try:
            r = self._s.post(
                f"{self.base_url}/remote/device/passthrough?deviceId={device_id}",
                data=json.dumps({
                    "deviceId": device_id,
                    "base64Input": b64_in,
                    "noOutput": False,
            ...
            if r.status_code == 401:
                ...
                if not self.login():
                    return None
                return self.fetch_passthrough()
            r.raise_for_status()
            j = r.json()
            if j.get("code") != 0 or not j.get("data"):
                return None
            raw = base64.b64decode(j["data"]).decode("utf-8", "ignore")
            fields = raw.split()
            ...
            out = {
                ...
                "grid_voltage": _f(self._PT_GRID_V),
                "output_freq": _f(self._PT_OUT_FREQ),
                "load_w": _f(self._PT_AC_POWER),
                "battery_voltage": _f(self._PT_BATT_V),
                "battery_soc": _f(self._PT_BATT_SOC),
                "pv_voltage": _f(self._PT_PV_V),
                "pv_power_w": _f(self._PT_PV_W),
            }
            if out["battery_voltage"] is None and out["pv_power_w"] is None:
                return None
            return out
        except Exception as e:  # noqa: BLE001
            log.error("Passthrough zlyhal: %s", e)
            return None
    ...
    def fetch(self) -> dict | None:
        ...
        if device and device.get("code") == 0 and isinstance(device.get("data"), dict):
            ...
            out["device_total"] = len(lst) if isinstance(lst, list) else None
        ...
        if flow and flow.get("code") == 0 and isinstance(flow.get("data"), dict):
            ...
            out["battery_power_kw"] = -_pick("batteryFlow") if _pick("batteryFlow") is not None else None
        ...
        if not any(k in out for k in (
            "pv_total_power", "daily_kwh", "total_kwh", "pv_active_kw",
            "load_power_kw", "inverter_producing_power", "daily_series",
        )):
            return None
        return out
