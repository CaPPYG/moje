...
def esp_online() -> bool:
    """True, ak ESP posielalo donedávna (do ~3-nasobku ocakavaneho intervalu)."""
    with _lock:
        le = _state["last_esp"]
    if not le:
        return False
    try:
        t = datetime.fromisoformat(le).timestamp()
    except ValueError:
        return False
    expected = cfg["esp"]["interval_active"] if is_active() else cfg["esp"]["interval_idle"]
    return (now() - t) < (expected * 3 + 5)
...
def passthrough_loop():
    pt = cfg.get("vendor", {}).get("passthrough", {}) or {}
    if not collector.enabled or not pt.get("enabled", False):
        log.info("Live passthrough vypnuty (vendor.passthrough.enabled=false)")
        return
    interval = float(pt.get("interval", 1))
    while True:
        with _pt_lock:
            data = collector.fetch_passthrough()
        if data is not None:
            with _lock:
                _state["snapshot"]["live"] = data
        time.sleep(interval)
